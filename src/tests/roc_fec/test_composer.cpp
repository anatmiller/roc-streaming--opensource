...
TEST(composer, align_footer) {
    ...
    CHECK(composer.align(slice, 0, Alignment) == status::StatusOK);
...
TEST(composer, align_outer_header) {
    ...
    Composer<RS8M_PayloadID, Source, Header> composer(NULL, arena);
    CHECK(composer.align(slice, OuterHeader, Alignment) == status::StatusOK);
...
TEST(composer, packet_size) {
    ...
    CHECK(composer.align(buffer, 0, Alignment) == status::StatusOK);
    CHECK(composer.prepare(*packet, buffer, PayloadSize) == status::StatusOK);
